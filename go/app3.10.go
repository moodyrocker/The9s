...
func main() {
	...
	router.POST("/form/update", formupdate)
...
func formupdate(w http.ResponseWriter, r *http.Request, _ httprouter.Params) {
	ul := url{}
...
func formediturl(w http.ResponseWriter, r *http.Request, _ httprouter.Params) {
	...
	id := filepath.Join("templates", "formedit.go.html")
	...
	rows, err := db.Query("select url_id, u_id, url, short_url from url where url_id = ?", url_id)
	if err != nil {
